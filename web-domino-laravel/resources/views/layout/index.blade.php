...
@if(request("is_iframe")!=1)
    ...
    @else
        ...
        <script>
             ...
             */
            function fnInitScrapedAndCycleFixPlanWarning() {
                $.ajax({
                    url: `{{ url('scrapedAndCycleFixPlanWarning') }}`,
                    type: 'get',
                    data: {},
                    async: true,
                    success: function (res) {
                        console.log(`{{ url('scrapedAndCycleFixPlanWarning') }} success:`, res);
                        let {scraped_statistics: scrapedStatistics, cycle_fix_plan_statistics: cycleFixPlanStatistics} = res['data'];
                        let total = 0;
                        ...
                        if (scrapedStatistics.length > 0) {
                            ...
                            html += `<thead>`;
                            html += `<tr>`;
                            html += `<th colspan="3" class="text-center"><h3>超期提醒</h3></th>`;
                            html += `</tr>`;
                            html += `<tr>`;
                            html += `<th>车站</th>`;
                            html += `<th>种类</th>`;
                            ...
                            scrapedStatistics.map(function (scrapedStatistic) {
                                ...
                                html += `<tr>`;
                                html += `<td><a href="${url}">${stationName}</a></td>`;
                        ...
                        if (cycleFixPlanStatistics.length > 0) {
                            ...
                            html += `<thead>`;
                            html += `<tr>`;
                            html += `<th colspan="3" class="text-center"><h3>周期修提醒</h3></th>`;
                            html += `</tr>`;
                            html += `<tr>`;
                            html += `<th>车站</th>`;
                            html += `<th>种类</th>`;
                    ...
                    error: function (err) {
                        console.log(`{{ url('scrapedAndCycleFixPlanWarning') }} fail:`, err);
                        if (err.status === 401) location.href = "{{ url('login') }}";
            ...
            $(function () {
                ...
                fnInitScrapedAndCycleFixPlanWarning();

                // 定时获取消息列表 (2秒）
                let i = 0;
                setInterval(function () {
                    // 每次页面打开最多十分钟获取消息，然后停止获取消息
                    if (i < parseInt("{{ env('SPAS_GET_MESSAGE_LIVE_TIME', 600) }}")) {
                        fnInitMessage();
                        i++;
                    }
                }, 2000);
